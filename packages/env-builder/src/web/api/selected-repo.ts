...
export async function selectRepo(store: EnvStore, req: IncomingMessage, res: ServerResponse): Promise<void> {
  const body = await readJson<{ repo?: string }>(req)
  if (!body.repo) throw new HttpError('env-builder: missing repo', 400)
  write(res, 200, 'application/json; charset=utf-8', JSON.stringify({ repo: store.selectRepo(body.repo) }))
}
